...
# =============== SONG PLAYER ===============
    ...
    # ✅ UPDATED KARAOKE TEMPLATE - MOBILE COMPATIBLE WITH FIXED DURATION
    karaoke_template = """
<!doctype html>
<html>
<head>
...
async function startRecordingAsync() {
    try {
        ...
        // Get microphone - SIMPLIFIED FOR MOBILE COMPATIBILITY
        let micStream;
        try {
            // Try with simpler constraints first (mobile friendly)
            ...
            console.log("Falling back to basic audio constraints:", err);
            micStream = await navigator.mediaDevices.getUserMedia({ 
                audio: true,
                video: false
            });
        ...
        // Simple gain nodes for mobile compatibility
        ...
        // MOBILE COMPATIBLE RECORDING SETTINGS
        ...
        try {
            ...
            console.log("Failed with mimeType, trying default:", e);
        ...
        mediaRecorder.onstop = () => {
            ...
            if (recordedChunks.length > 0) {
                ...
                finalStatus.innerText = `Recording Complete! (${Math.round(recordingDuration)}s)`;
                
                // Set download link
                const songName = "%%SONG_NAME%%".replace(/[^a-zA-Z0-9]/g, '_');
                const fileName = `${songName}_recording_${Date.now()}.webm`;
                ...
                status.innerText = "❌ No recording data available";
                resetUIOnError();
            }
        };
        ...
        // Auto-stop based on song duration
...
function stopRecording() {
    ...
    status.innerText = `✅ Recording saved (${Math.round(recordingDuration)}s)`;
}
...
document.addEventListener('visibilitychange', function() {
    if (document.hidden && isRecording) {
        // Auto-stop recording if page becomes hidden
        stopRecording();
    }
});
...
window.addEventListener('load', function() {
    console.log("Karaoke Player Loaded - Mobile Compatible");
    status.innerText = "Ready 🎤";
