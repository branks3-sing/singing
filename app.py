...
let audioContext, micSource, accSource, micGain, accGain, micCompressor, micFilter;
...
playBtn.onclick = function() {
    if (originalAudio.paused) {
        ...
        originalAudio.play().then(() => {
            ...
            status.innerText = "❌ Cannot play audio";
        });
    } else {
        originalAudio.pause();
        originalAudio.currentTime = 0;
...
/* ================== RECORD - WITH VOICE CLARITY IMPROVEMENTS ================== */
...
async function startRecordingAsync() {
    try {
        ...
        // HIGH-QUALITY MICROPHONE SETTINGS FOR CLARITY
        const micStream = await navigator.mediaDevices.getUserMedia({ 
            audio: {
                echoCancellation: true,
                noiseSuppression: true,
                autoGainControl: false,  // Disabled for better control
                channelCount: 1,
        ...
        // AUDIO PROCESSING FOR VOICE CLARITY
        micGain = audioContext.createGain();
        micGain.gain.value = 2.5;  // Slightly higher gain
        
        // Create compressor for consistent voice levels
        micCompressor = audioContext.createDynamicsCompressor();
        micCompressor.threshold.setValueAtTime(-24, audioContext.currentTime);
        micCompressor.knee.setValueAtTime(30, audioContext.currentTime);
        micCompressor.ratio.setValueAtTime(12, audioContext.currentTime);
        micCompressor.attack.setValueAtTime(0.003, audioContext.currentTime);
        micCompressor.release.setValueAtTime(0.25, audioContext.currentTime);
        
        // Create high-pass filter to remove low-frequency rumble
        micFilter = audioContext.createBiquadFilter();
        micFilter.type = "highpass";
        micFilter.frequency.setValueAtTime(80, audioContext.currentTime);
        micFilter.Q.setValueAtTime(1, audioContext.currentTime);
        
        // Create EQ for voice clarity (boost mid-high frequencies)
        const micEQ = audioContext.createBiquadFilter();
        micEQ.type = "peaking";
        micEQ.frequency.setValueAtTime(2000, audioContext.currentTime);
        micEQ.gain.setValueAtTime(6, audioContext.currentTime);  // Boost 2kHz
        micEQ.Q.setValueAtTime(1, audioContext.currentTime);
        
        accGain = audioContext.createGain();
        accGain.gain.value = 0.2;  // Lower accompaniment volume
        
        const destination = audioContext.createMediaStreamDestination();
        
        // Connect microphone chain: source -> gain -> filter -> compressor -> EQ -> destination
        micSource.connect(micGain);
        micGain.connect(micFilter);
        micFilter.connect(micCompressor);
        micCompressor.connect(micEQ);
        micEQ.connect(destination);
        ...
        const mimeType = MediaRecorder.isTypeSupported('video/webm;codecs=vp9,opus') 
            ? 'video/webm;codecs=vp9,opus'  // Better codec
        ...
        mediaRecorder = new MediaRecorder(combinedStream, {
            mimeType: mimeType,
            videoBitsPerSecond: 2500000,      // Higher video bitrate
            audioBitsPerSecond: 192000        // Higher audio bitrate for clarity
        });
...
window.addEventListener('load', () => {
    console.log("Karaoke Player Loaded - Enhanced Voice Clarity");
    status.innerText = "Ready 🎤";
});
</script>
</body>
